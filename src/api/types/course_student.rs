...
pub struct CourseStudent {
    ...
    #[serde(default)]
    pub course: CourseStudentCourse,
    ...
    #[serde(default)]
    pub user: CourseStudentUser,
}

impl CourseStudent {
    pub fn builder() -> CourseStudentBuilder {
        <CourseStudentBuilder as Default>::default()
    }
}

#[derive(Clone, PartialEq, Default, Debug)]
#[non_exhaustive]
pub struct CourseStudentBuilder {
    completed_lessons_count: Option<i64>,
    completion_rate: Option<f64>,
    course: Option<CourseStudentCourse>,
    ...
    user: Option<CourseStudentUser>,
}

impl CourseStudentBuilder {
    ...
    pub fn course(mut self, value: CourseStudentCourse) -> Self {
    ...
    pub fn user(mut self, value: CourseStudentUser) -> Self {
        self.user = Some(value);
        self
    }

    /// Consumes the builder and constructs a [`CourseStudent`].
    /// This method will fail if any of the following fields are not set:
    /// - [`completed_lessons_count`](CourseStudentBuilder::completed_lessons_count)
    /// - [`completion_rate`](CourseStudentBuilder::completion_rate)
    /// - [`course`](CourseStudentBuilder::course)
    /// - [`first_interaction_at`](CourseStudentBuilder::first_interaction_at)
    /// - [`id`](CourseStudentBuilder::id)
    /// - [`last_interaction_at`](CourseStudentBuilder::last_interaction_at)
    /// - [`total_lessons_count`](CourseStudentBuilder::total_lessons_count)
    /// - [`user`](CourseStudentBuilder::user)
    pub fn build(self) -> Result<CourseStudent, BuildError> {
        Ok(CourseStudent {
